...
EXECUTE the following phases SEQUENTIALLY. COMPLETE each phase entirely before proceeding to the next:
    <phase number="1" name="Coding File Change Discovery">
        <task id="1.1" title="For Each Changed File In Task Management Tool">
        EXECUTE #review_new_code_with_filter tool to identify coding standard violations with the following parameters:
        ```json
        {
            "gitDiffOptions": "origin/<CURRENT_GIT_BRANCH>..HEAD",
            "gitDiffFileFilter": [":*.ts*", ":(exclude)*.spec.ts"],
        ...
        <task id="1.3" title="Address File-Level Identified Violations">
        ...
        <task id="1.4" title="Execute Standardized Commit Workflow">
            READ and FOLLOW `.github/prompts/chains/git-commit.prompt.md` workflow EXACTLY to commit changes with concise yet meaningful message
        </task>
    </phase>
    <phase number="2" name="Testing File Change Discovery">
        <task id="2.1" title="For Each Changed File In Task Management Tool">
            EXECUTE #review_new_code_with_filter tool to identify coding standard violations with the following parameters:
            ```json
            {
                "gitDiffOptions": "--cached",
                "gitDiffFileFilter": [":*.spec.ts", ":*.test.ts"],
                "CodingStandardUris": ".github/docs/testing-convention.md"
            }
            ```
        </task>
        <task id="2.2" title="Add Files to Task Management Tool">
            - IMMEDIATELY add all files returned from tool to the task management tool for processing file by file 
            - Task name MUST be the file name found from the tool, description MUST contain meaningful context for code refactoring analysis
        </task>
        <task id="2.3" title="Address File-Level Identified Violations">
           ...
           MAKE SURE:
           - IMPLEMENT fixes by addressing EACH identified issue systematically through refactoring
           - VERIFY quality by ensuring ALL corrections follow best practices defined in the .github/docs/testing-convention.md
           - PRESERVE functionality by confirming that ALL changes maintain existing functionality
        </task>
        <task id="2.4" title="Execute Standardized Commit Workflow">
            READ and FOLLOW `.github/prompts/chains/git-commit.prompt.md` workflow EXACTLY to commit changes with concise yet meaningful message
        </task>
    </phase>
</phases>
...
- NEVER deviate from the git-commit.prompt.md workflow during commitment process
...
<checklist>
    ...
    <item>✅ EVERY coding or testing standard violation addressed through comprehensive refactoring</item>
    ...
    <item>✅ Quality improvements committed using standardized workflow process</item>
    <item>✅ Concise commit message accurately describes implemented improvements</item>
