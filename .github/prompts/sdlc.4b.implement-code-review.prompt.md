...
EXECUTE the following phases SEQUENTIALLY. COMPLETE each phase entirely before proceeding to the next:
    <phase number="1" name="Discovery & Planning">
        <task id="1.1" title="Identify Incomplete Tasks">
            READ `.github/plans/[CURRENT_GIT_BRANCH]/refactor.plan.md` and IDENTIFY ALL incomplete refactor files (marked with `[ ]`) to create a prioritized list of refactoring tasks
        </task>
    </phase>
    <phase number="2" name="Implementation Execution">
        ...
        <task id="2.2" title="Execute File-Level Refactoring with Immediate Commit">
            ...
            2. AFTER completing ALL refactoring for the file, MARK the completed items in `refactor.plan.md` (change `[ ]` to `[x]`) for that file
            3. READ and FOLLOW `.github/prompts/chains/git-commit.prompt.md` to commit changes with message:
...
AFTER COMPLETING all phases: EXECUTE these tasks:
    <task title="Execute Reflection Workflow">
        READ and FOLLOW `.github/prompts/reflections/sdlc.4.implement-code-review.reflection.md` workflow
    </task>
</postWorkflowTasks>
